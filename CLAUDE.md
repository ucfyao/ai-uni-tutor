...
- **ActionResult<T> mandatory for all Server Actions:** Every server action returns `ActionResult<T>` — never throws. Error flow: Repo throws `DatabaseError` → Service may wrap with `AppError` → Action catches with `mapError()` → Component checks `.success`. Exception: `FormActionState` actions for `useActionState` forms. (#329)
