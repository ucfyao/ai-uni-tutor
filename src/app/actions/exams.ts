...
export async function renameExamPaper(
  ...
  try {
    ...
    await getExamPaperService().updatePaperMeta(parsed.paperId, { title: parsed.title });
...
export async function fetchExamQuestions(
  ...
  try {
    ...
    const questions = await getExamPaperService().getQuestionsByPaperId(parsed.paperId);
...
export async function updateSingleExamQuestion(
  ...
  try {
    ...
    await examService.updateQuestion(parsed.questionId, fields);
