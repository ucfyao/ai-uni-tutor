...
interface NewSessionModalProps {
  opened: boolean;
  onClose: () => void;
  onStart: (course: Course, mode: TutoringMode) => void;
}
...
const NewSessionModal: React.FC<NewSessionModalProps> = ({ opened, onClose, onStart }) => {
  ...
  const FormRow = ({ icon: Icon, label, color, children, active }: { icon: React.ElementType, label: string, color: string, children: React.ReactNode, active: boolean }) => (
    <Box 
      style={{ 
        borderRadius: '16px', 
    ...
    >
      <Group gap={0} wrap="nowrap" align="stretch" w="100%" h="100%">
        ...
        <Center w={60} style={{ borderRight: active ? `1px solid var(--mantine-color-gray-1)` : 'none' }}>
            <ThemeIcon 
        ...
        {/* Content Section */}
        <Box style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center' }} px="md">
          <Text 
            size="10px" 
            fw={700} 
            tt="uppercase" 
          ...
          <Box style={{ position: 'relative', width: '100%' }}>
            {children}
            <Box style={{ position: 'absolute', right: 0, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none', color: 'var(--mantine-color-gray-4)' }}>
      ...
      <Stack gap={24}>
        <Group justify="space-between" align="start" mb={2}>
          <Box>
            ...
            <Text size="sm" c="gray.5" fw={500} mt={4}>Configure your study session</Text>
          </Box>
          <UnstyledButton 
            onClick={onClose} 
            w={36} h={36} 
