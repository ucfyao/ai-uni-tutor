...
export class ExamPaperRepository implements IExamPaperRepository {
  ...
  async deleteQuestionsByPaperId(paperId: string): Promise<void> {
    ...
    if (error) {
      throw new DatabaseError(`Failed to delete questions by paper: ${error.message}`, error);
    }
  }
  ...
  async getStats(paperIds: string[]): Promise<
    Map<
      string,
      {
        itemCount: number;
  ...
  > {
    ...
    const { data, error } = await supabase
      .from('exam_questions')
      .select('paper_id, parent_question_id, answer, metadata')
      .in('paper_id', paperIds);

    if (error) {
      throw new DatabaseError(`Failed to fetch exam stats: ${error.message}`, error);
    }
    ...
    for (const row of (data ?? []) as Array<Record<string, unknown>>) {
      ...
      if ((row.answer as string)?.trim()) stat.withAnswer++;

      const metadata = row.metadata as Record<string, unknown> | null;
      if (metadata && Array.isArray(metadata.warnings) && metadata.warnings.length > 0) {
        stat.warningCount++;
      }
    }
